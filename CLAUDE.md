...
- `execute_ffmpeg()` is the single encode routine (probe → ffmpeg → hw→libx264 fallback → atomic `os.replace`, skipped for workers), used by local slots and workers alike. The worker probes subtitles/audio itself (it has the mount), so the claim only carries source/output path, preset dict and duration
...
- DB schema is now **version 8** (`localUri` added to `DownloadEntity` — the SAF document URI the file was written to, so deletion doesn't have to re-find it by name; v7 added `played`, which `DownloadDao.updatePlayed()`/`MetadataRefreshWorker` write but was missing from the entity, breaking CI's KSP query validation with `no such column: played`). Destructive migration still wipes existing download records on upgrade — a deliberate pre-distribution choice, documented at the `fallbackToDestructiveMigration` call in `AppModule.kt`. `exportSchema = true` now writes schemas to `android/app/schemas/` (via `room.schemaLocation` in `build.gradle.kts`) so a real migration chain can be written when the app ships
