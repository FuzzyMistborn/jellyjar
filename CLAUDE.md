...
- **Auto-play "Up next" countdown**: playback ending no longer cuts straight to the next episode; `UpNextCard` shows the resolved episode ("S02E04 · Title", from the new `NextEpisodeTarget.title`) with a 10s countdown plus Play now / Cancel. Deliberately not behind its own setting — the existing "Auto-play Next Episode" toggle still governs whether anything is resolved at all
